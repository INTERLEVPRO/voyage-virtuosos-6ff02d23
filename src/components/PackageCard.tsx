      ...
      <div className="flex flex-1 flex-col gap-3 p-4 sm:gap-4 sm:p-5 md:flex-row md:items-stretch">
        ...
        <div className="flex flex-col justify-between gap-4 md:w-56 md:border-l md:border-border md:pl-5">
          <ul className="space-y-1.5 text-sm">
            {ratings.length === 0 ? (
            <li className="flex flex-col gap-2 rounded-xl border border-border/60 bg-secondary/40 px-3 py-2.5">
              <div className="flex items-center gap-1.5">
                <ThumbsUp className="h-3.5 w-3.5 text-emerald-500 shrink-0" />
                <span className="text-xs font-semibold text-foreground">KI-geprüfte Auswahl</span>
              </div>
              <p className="text-[11px] leading-snug text-muted-foreground">
                Passend zu Budget, Reisedauer &amp; Interessen — curated by AI.
              </p>
              <div className="flex flex-wrap gap-1 pt-0.5">
                {["Budget ✓", "Reisedauer ✓", "Komfort ✓"].map((label) => (
                  <span key={label} className="rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-medium text-emerald-700 border border-emerald-200">
                    {label}
                  </span>
                ))}
              </div>
