...
export function PriceConfirmation({
  ...
  const diffLabel =
    priceDifference === 0
      ? "0 €"
      : `${priceDifference > 0 ? "+" : ""}${priceDifference.toLocaleString(locale)} €`;
  const diffClass =
    priceDifference > 0
      ? "text-destructive"
      : priceDifference < 0
  ...
  return (
    <div className="mt-4 rounded-2xl border border-accent/40 bg-accent/5 p-5 shadow-soft">
      <h3 className="font-display text-lg text-primary">{t("price.title")}</h3>
      ...
      <div className="mt-3 flex items-center gap-3 text-sm">
        ...
        <span className="font-display text-xl text-primary">
          {newPrice.toLocaleString(locale)} €
        </span>
      ...
      <p className="mt-4 text-sm font-medium text-primary">{t("price.question")}</p>
